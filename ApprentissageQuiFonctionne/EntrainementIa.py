...
class TrackmaniaEnvironment:
    ...
    def calculate_reward(self, current_position):
        """Calcule la récompense basée sur la progression et le temps"""
        reward = 0
        ...
        # Récompense pour la progression
        if self.last_position is not None:
            # Distance parcourue
            distance = np.linalg.norm(current_position - self.last_position)
            reward += distance * 10.0  # Augmenté pour donner plus d'importance à la progression

            # Bonus de vitesse
            speed = np.linalg.norm(self.velocity)
            reward += speed * 0.5  # Bonus pour maintenir une vitesse élevée

            # Pénalité si la voiture est bloquée
            if distance < 0.01:  # Seuil ajusté
                self.frames_stuck += 1
